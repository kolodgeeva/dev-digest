...
Append-only notes the previous session leaves for the next, scoped to the
deterministic browser e2e suite (real stack, no LLM). Maintained by the
...
explicitly; split into domain files (e.g. `INSIGHTS-flakes.md`) past ~200 entries.
