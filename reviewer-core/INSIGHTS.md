...
Append-only notes the previous session leaves for the next, scoped to the review
engine (diff → prompt → LLM → findings) and grounding gate. Maintained by the
`engineering-insights` skill — see `.claude/skills/engineering-insights/SKILL.md`.
...
explicitly; split into domain files (e.g. `INSIGHTS-prompts.md`) past ~200 entries.
